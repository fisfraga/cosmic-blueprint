...
// The HD Wheel runs counter-clockwise from the "Rave New Year" point
// This mapping is based on tropical zodiac positions
const HD_GATES = [
...
function calculateLongitude(planetId, date) {
  const body = BODY_MAP[planetId];

  try {
    // IMPORTANT: Use GeoVector + Ecliptic for ALL planets to get GEOCENTRIC coordinates
    // EclipticLongitude returns heliocentric coordinates for planets, which is wrong for astrology
...
function calculatePositions(date) {
  ...
  for (const planetId of PLANET_ORDER) {
    result[planetId] = calculateLongitude(planetId, date);
  }
  return result;
}
...
function getGateByDegree(absoluteDegree) {
...
function calculateEarthPosition(sunLongitude) {
  return (sunLongitude + 180) % 360;
}

function calculateDesignDate(birthDate, birthSunLongitude) {
...
function runValidation() {
  ...
  for (const planetId of [...PLANET_ORDER, 'earth']) {
    const lon = planetId === 'earth' ? natalEarth : natalPositions[planetId];
  ...
  for (const planetId of [...PLANET_ORDER, 'earth']) {
    const lon = planetId === 'earth' ? designEarth : designPositions[planetId];
    const zodiac = longitudeToZodiac(lon);
    const gate = getGateByDegree(lon);
    const gateStr = gate ? `Gate ${gate.gateNumber}.${gate.line}` : 'N/A';
    console.log(`  ${planetId.padEnd(10)} ${zodiac.sign.padEnd(12)} ${String(zodiac.degree).padStart(2)}°${String(zodiac.minute).padStart(2)}'  ${gateStr}`);
  ...
  for (const [sphereKey, mapping] of Object.entries(SPHERE_PLANET_MAP)) {
    ...
    if (mapping.planetId === 'earth') {
      ...
      lon = positions[mapping.planetId];
    }

    const calculated = getGateByDegree(lon);
    const stored = FELIPE_STORED[sphereKey];
    ...
    if (!match) {
      ...
      mismatches++;
      if (calculated) {
        corrections.push({ sphereKey, calculated, stored });
      }
    } else {
      matches++;
    }
  ...
  if (corrections.length > 0) {
    ...
    console.log('='.repeat(70));
    console.log('');
    console.log('Use these values to update the profile in ProfileContext.tsx:');
    console.log('');

    for (const { sphereKey, calculated } of corrections) {
      console.log(`${sphereKey}: {`);
      console.log(`  sphereName: '${SPHERE_NAMES[sphereKey]}',`);
      console.log(`  geneKeyId: 'gk-${calculated.gateNumber}',`);
      console.log(`  geneKeyNumber: ${calculated.gateNumber},`);
      console.log(`  line: ${calculated.line},`);
      console.log(`  planetarySource: '${SPHERE_PLANET_MAP[sphereKey].isDesign ? 'Pre-Natal / Design' : 'Natal'} ${SPHERE_PLANET_MAP[sphereKey].planetId.charAt(0).toUpperCase() + SPHERE_PLANET_MAP[sphereKey].planetId.slice(1)}',`);
      console.log(`},`);
    }
  }
}
