...
### CSP Headers
- [x] `vercel.json` updated to add CSP header on all routes:
  ```
  ...
  ```
- [x] App still loads and functions correctly after CSP is applied (no blocked resources)
- [x] No inline script errors in browser console

### API Key Documentation
- [x] `docs/guides/api-key-security.md` created documenting:
...
- [x] `api/claude.ts` contains no `console.log` statements that could expose the key or request details

## Tasks

- [x] Read `vercel.json` and `api/claude.ts`
- [x] Add CSP header block to `vercel.json`
- [x] Test CSP locally (if possible): `npm run build && npm run preview` with network tab open
- [x] Check `api/claude.ts` for any logging that could expose the API key
- [x] Create `docs/guides/api-key-security.md`
