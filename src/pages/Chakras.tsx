...
export function Chakras() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <h2 className="font-serif text-xl text-neutral-300 mb-4">The Energy Column</h2>
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          {/* SVG Spine */}
          <div className="flex-shrink-0 rounded-2xl border border-neutral-800 bg-neutral-900/40 p-4">
            <p className="text-xs text-neutral-600 text-center mb-2 uppercase tracking-widest">
...
function ChakraRow({ chakra }: { chakra: Chakra }) {
  return (
    <Link
      ...
      className="flex items-center gap-4 p-4 bg-neutral-900/50 rounded-xl border border-neutral-800 hover:border-emerald-500/40 transition-all group"
    >
      {/* Color dot */}
      <div
      ...
      <div className="w-8 text-center flex-shrink-0">
        <span className="text-neutral-500 text-sm font-mono">{chakra.number}</span>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-neutral-500 text-sm italic">{chakra.sanskritName}</span>
        </div>
        <p className="text-neutral-500 text-xs truncate">{chakra.lifeTheme}</p>
      </div>

      {/* Houses */}
      <div className="flex-shrink-0 text-right">
        <div className="text-neutral-500 text-xs">House{chakra.relatedHouses.length > 1 ? 's' : ''}</div>
        <div className="text-neutral-400 text-sm">{chakra.relatedHouses.join(', ')}</div>
      </div>
    </Link>
  );
}
...
function ChakraCard({ chakra }: { chakra: Chakra }) {
  return (
    <Link
      to={`/chakras/${chakra.id}`}
      className="bg-neutral-900/50 rounded-xl p-5 border border-neutral-800 hover:border-emerald-500/50 hover:bg-neutral-900 transition-all group"
    >
      {/* Color header */}
      <div
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          <h3 className="font-serif text-lg text-white group-hover:text-emerald-300 transition-colors">
            {chakra.name}
          </h3>
      ...
      <p className="text-neutral-400 text-sm mb-3">{chakra.archetype}</p>

      {/* Constricted → Flowing → Radiant */}
      <div className="space-y-1 text-sm">
        <div className="flex items-center gap-2">
          <span className="w-16 text-neutral-500 text-xs">Constricted:</span>
          <span className="text-red-400">{chakra.constricted.name}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-16 text-neutral-500 text-xs">Flowing:</span>
          <span className="text-emerald-400">{chakra.flowing.name}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-16 text-neutral-500 text-xs">Radiant:</span>
