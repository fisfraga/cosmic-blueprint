            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-serif text-lg text-white">{circuit} Circuit</h3>
                  <span className="text-sm text-neutral-500">{channels.length} channels</span>
                </div>
                <p className="text-neutral-400 text-sm mb-3">{circuitDescriptions[circuit]}</p>
                <div className="flex flex-wrap gap-1">
                  {channels.slice(0, 6).map(ch => (
                    <span
                      key={ch.id}
                      className="text-xs text-humandesign-400 bg-neutral-800 px-2 py-0.5 rounded"
                    >
                      {ch.gate1Number}-{ch.gate2Number}
                    </span>
                  ))}
                  {channels.length > 6 && (
                    <span className="text-xs text-neutral-500">+{channels.length - 6} more</span>
                  )}
                </div>
              </div>
            );
...
function ChannelCard({ channel }: { channel: HDChannel }) {
  ...
  return (
    ...
    >
      ...
      {/* Gene Key bridge */}
      {(gk1 || gk2) && (
        <div className="pt-2 border-t border-neutral-800 flex flex-col gap-0.5">
          {gk1 && (
