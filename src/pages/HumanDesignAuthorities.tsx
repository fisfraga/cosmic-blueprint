    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <p className="text-theme-text-secondary max-w-3xl">
          ...
          communicates what's correct for you. While Strategy tells you <em className="text-theme-text-secondary">how to engage</em> with
          life, Authority tells you <em className="text-theme-text-secondary">how to make decisions</em>.
        </p>
            ...
            return (
              <div key={authority.id} className="flex gap-4 items-start">
                <div className="w-16 text-center">
                  ...
                  <p className="text-xs text-theme-text-tertiary mt-1">{authority.percentage}</p>
                </div>
                <div className="flex-1">
                  ...
                  {authority.alternativeNames.length > 0 && (
                    <span className="text-theme-text-tertiary text-xs ml-2">
                      ({authority.alternativeNames.join(', ')})
                    </span>
                  )}
                  <p className="text-theme-text-secondary mt-1">{authority.decisionProcess.slice(0, 150)}...</p>
                  <div className="flex gap-4 mt-2 text-xs">
                    ...
                    <span className="text-theme-text-tertiary">
                      Timeframe: <span className="text-theme-text-secondary">{authority.timeframe}</span>
                    </span>
                  </div>
                </div>
              </div>
      ...
      <section className="bg-surface-base/50 rounded-xl p-6 border border-theme-border-subtle">
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="flex items-center gap-4 py-2 border-b border-theme-border-subtle last:border-0">
              <span className="w-6 h-6 flex items-center justify-center bg-humandesign-500/20 text-humandesign-400 rounded text-sm">
                {i + 1}
              </span>
              <div className="flex-1">
                <span className="text-theme-text-primary">{item.name}</span>
                <span className="text-theme-text-tertiary text-sm ml-2">— {item.desc}</span>
              </div>
              <span className="text-theme-text-tertiary text-xs">{item.pct}</span>
            </div>
          ))}
        </div>
      </section>
