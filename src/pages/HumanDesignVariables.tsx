...
function VariableCard({ variable }: { variable: HDVariableEntity }) {
  ...
  return (
    <div className="bg-surface-base/50 rounded-xl border border-theme-border-subtle overflow-hidden">
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full p-4 text-left hover:bg-surface-raised/30 transition-colors"
      >
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <span className="text-xl">{variable.symbol}</span>
            <div>
              <h3 className="font-mediumtext-theme-text-primary">{variable.name}</h3>
          ...
          <svg
            className={`w-5 h-5 text-theme-text-secondary transition-transform ${expanded ? 'rotate-180' : ''}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-theme-border-subtle pt-4 space-y-4">
          <p className="text-theme-text-secondary text-sm leading-relaxed">{variable.description}</p>

          {/* Keywords */}
          <div>
            <p className="text-xs text-theme-text-tertiary uppercase tracking-wider mb-2">Keywords</p>
            <div className="flex flex-wrap gap-2">
              {variable.keywords.map((keyword, i) => (
                <span key={i} className="px-2 py-1 bg-surface-raised rounded text-xs text-theme-text-secondary">
                  {keyword}
                </span>
              ))}
            </div>
          ...
          {variable.physicalApplication && (
            <div>
              <p className="text-xs text-theme-text-tertiary uppercase tracking-wider mb-2">Physical Application</p>
              <p className="text-theme-text-secondary text-sm">{variable.physicalApplication}</p>
            </div>
          )}
          {variable.mentalApplication && (
            <div>
              <p className="text-xs text-theme-text-tertiary uppercase tracking-wider mb-2">Mental Application</p>
              <p className="text-theme-text-secondary text-sm">{variable.mentalApplication}</p>
            </div>
          )}
          ...
          {variable.environmentalSupport && (
            <div>
              <p className="text-xs text-theme-text-tertiary uppercase tracking-wider mb-2">Environmental Support</p>
              <p className="text-theme-text-secondary text-sm">{variable.environmentalSupport}</p>
            </div>
          )}
          ...
          {variable.idealCharacteristics && variable.idealCharacteristics.length > 0 && (
            <div>
              <p className="text-xs text-theme-text-tertiary uppercase tracking-wider mb-2">Ideal Characteristics</p>
              <ul className="text-theme-text-secondary text-sm list-disc list-inside space-y-1">
          ...
          {variable.perceptionStrength && (
            <div>
              <p className="text-xs text-theme-text-tertiary uppercase tracking-wider mb-2">Perception Strength</p>
              <p className="text-theme-text-secondary text-sm">{variable.perceptionStrength}</p>
            </div>
          )}
          ...
          {variable.healthyExpression && (
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-theme-text-secondary text-sm">{variable.healthyExpression}</p>
              </div>
              {variable.unhealthyExpression && (
                <div>
                  <p className="text-xs text-rose-400 uppercase tracking-wider mb-2">Unhealthy Expression</p>
                  <p className="text-theme-text-secondary text-sm">{variable.unhealthyExpression}</p>
                </div>
              )}
            </div>
          )}
          ...
          {/* Challenges */}
          <div>
            <p className="text-xs text-theme-text-tertiary uppercase tracking-wider mb-2">Challenges</p>
            <ul className="text-theme-text-secondary text-sm list-disc list-inside space-y-1">
          ...
          <div className="bg-surface-overlay rounded-lg p-4">
            <p className="text-xs text-theme-text-tertiary uppercase tracking-wider mb-2">Practical Guidance</p>
            <p className="text-theme-text-primary text-sm leading-relaxed">{variable.practicalGuidance}</p>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <section className="space-y-4">
      <div className={`bg-gradient-to-br ${categoryColors[category]} rounded-xl p-6 border`}>
        ...
        <p className="text-theme-text-secondary text-sm">{categoryInfo?.description}</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-sm font-medium text-theme-text-secondary">←</span>
            <h3 className="text-sm font-medium text-theme-text-secondary">{arrowLabels[category].left}</h3>
          </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-sm font-medium text-theme-text-secondary">→</span>
            <h3 className="text-sm font-medium text-theme-text-secondary">{arrowLabels[category].right}</h3>
          </div>
          <div className="space-y-3">
      ...
      <section className="bg-surface-base/50 rounded-xl p-6 border border-theme-border-subtle">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 bg-surface-raised rounded-full flex items-center justify-center text-lg">←</div>
            <div>
              <h4 className="font-medium text-theme-text-primary mb-1">Left Arrow (Strategic)</h4>
          ...
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 bg-surface-raised rounded-full flex items-center justify-center text-lg">→</div>
            <div>
              <h4 className="font-medium text-theme-text-primary mb-1">Right Arrow (Receptive)</h4>
              <p className="text-theme-text-secondary text-sm">
