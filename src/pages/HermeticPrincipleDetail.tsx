  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <header className="text-center py-8">
        ...
        <div className="text-neutral-500 text-sm mb-2">Principle {principle.number} of 7 · {principle.latinName}</div>
      ...
      <section className="bg-gradient-to-br from-amber-500/10 to-amber-600/5 rounded-xl p-6 border border-amber-500/20">
        ...
        <p className="text-neutral-300 leading-relaxed">{principle.essence}</p>
      </section>

      {/* Description */}
      {principle.description && (
        <section className="bg-neutral-900/50 rounded-xl p-6 border border-neutral-800">
          <p className="text-neutral-300 leading-relaxed">{principle.description}</p>
        </section>
      )}
      ...
      <section className="bg-gradient-to-br from-red-500/10 to-red-600/5 rounded-xl p-6 border border-red-500/20">
        ...
        <p className="text-neutral-300 leading-relaxed">{principle.shadow.expression}</p>
      </section>
      ...
      <section className="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 rounded-xl p-6 border border-emerald-500/20">
        ...
        <p className="text-neutral-300 leading-relaxed">{principle.gift.expression}</p>
      </section>
      ...
      <section className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 rounded-xl p-6 border border-purple-500/20">
        ...
        <p className="text-neutral-300 leading-relaxed">{principle.siddhi.expression}</p>
      </section>

      {/* Applications */}
      <section className="grid md:grid-cols-2 gap-4">
        <div className="bg-neutral-900/50 rounded-xl p-6 border border-neutral-800">
          <h2 className="font-serif text-lg mb-3 text-neutral-200">In Astrology</h2>
          <p className="text-neutral-300 text-sm leading-relaxed">{principle.astrologyApplication}</p>
        </div>
        <div className="bg-neutral-900/50 rounded-xl p-6 border border-neutral-800">
          <h2 className="font-serif text-lg mb-3 text-neutral-200">In Gene Keys</h2>
          <p className="text-neutral-300 text-sm leading-relaxed">{principle.geneKeysApplication}</p>
        </div>
      </section>

      {/* Practice */}
      <section className="bg-neutral-900/50 rounded-xl p-6 border border-amber-500/10">
        <h2 className="font-serif text-lg mb-3 text-amber-300/80">Practice</h2>
        <p className="text-neutral-300 leading-relaxed">{principle.practice}</p>
      </section>
          ...
          <div className="flex flex-wrap gap-3">
            {relatedChakras.map((chakra) => chakra && (
              ...
              >
                ...
                <div>
                  <p className="text-white text-sm">{chakra.name}</p>
                  <p className="text-neutral-500 text-xs italic">{chakra.sanskritName}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      <section className="bg-gradient-to-br from-neutral-900/80 to-neutral-800/30 rounded-xl p-6 border border-neutral-700">
        ...
        <p className="text-neutral-300 italic leading-relaxed">{principle.contemplativeQuestion}</p>
      </section>
      ...
      <section className="flex items-center justify-between pt-4 border-t border-neutral-800">
        {prevPrinciple ? (
          <Link
            to={`/hermetic/${prevPrinciple.id}`}
            className="flex items-center gap-2 text-neutral-400 hover:text-amber-300 transition-colors"
          >
            <span>←</span>
            <span className="text-sm">{prevPrinciple.name}</span>
          </Link>
        ) : <div />}
        <Link to="/hermetic" className="text-neutral-500 hover:text-white transition-colors text-sm">
          All Principles
        </Link>
        {nextPrinciple ? (
          <Link
