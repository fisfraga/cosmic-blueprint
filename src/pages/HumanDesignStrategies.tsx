    ...
    <div className="space-y-8">
        ...
        {allStrategies.map((strategy) => {
          ...
          return (
            ...
            >
                    ...
                    <div className="flex gap-2 mt-1">
                      {types.map(type => (
                        <Link
                          ...
                          className={`text-xs px-2 py-0.5 ${colors.text} bg-surface-base/50 rounded-full hover:bg-surface-base/70 transition-colors`}
                        >
                          {type?.name}
                        </Link>
                      ))}
              ...
              {/* Description */}
              <p className="text-theme-text-secondary leading-relaxed mb-4">{strategy.description}</p>

              {/* Practical Guidance */}
              <div className="bg-surface-base/50 rounded-lg p-4 mb-4">
                <h3 className={`text-sm font-medium ${colors.text} mb-2`}>Practical Guidance</h3>
                <p className="text-sm text-theme-text-secondary">{strategy.practicalGuidance}</p>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-4">
                <div className="bg-emerald-900/20 rounded-lg p-4 border border-emerald-500/20">
                  ...
                  <p className="text-sm text-theme-text-secondary">{strategy.signOfCorrectness}</p>
                </div>
                <div className="bg-red-900/20 rounded-lg p-4 border border-red-500/20">
                  <h3 className="text-sm font-medium text-red-400 mb-2">Sign of Incorrectness</h3>
                  <p className="text-sm text-theme-text-secondary">{strategy.signOfIncorrectness}</p>
                </div>
              </div>
                ...
                <div className="mb-4">
                  ...
                  <ul className="space-y-1.5">
                    {strategy.examples.map((example, idx) => (
                      <li key={idx} className="text-sm text-theme-text-secondary flex items-start gap-2">
              ...
              {strategy.commonMisunderstandings && strategy.commonMisunderstandings.length > 0 && (
                <div className="bg-surface-overlay rounded-lg p-4">
                  <h3 className="text-sm font-medium text-theme-text-secondary mb-2">Common Misunderstandings</h3>
                  <ul className="space-y-1.5">
                    {strategy.commonMisunderstandings.map((misunderstanding, idx) => (
                      <li key={idx} className="text-sm text-theme-text-tertiary flex items-start gap-2">
              ...
              {strategy.keywords && strategy.keywords.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {strategy.keywords.map((keyword, idx) => (
                    <span
                      key={idx}
                      className={`text-xs px-2 py-1 ${colors.text} bg-surface-base/50 rounded-full`}
                    >
                      {keyword}
                    </span>
                  ))}
      ...
      <section className="flex gap-4">
        ...
        >
          ...
          <p className="text-sm text-theme-text-secondary mt-1">Learn about the five Human Design Types</p>
        </Link>
        <Link
        ...
        >
          ...
          <p className="text-sm text-theme-text-secondary mt-1">Explore decision-making authorities</p>
        </Link>
      </section>
    </div>
  );
