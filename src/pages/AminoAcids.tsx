    ...
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4">
          ...
          <div className="text-xs text-neutral-500 mt-1">Must be obtained from diet</div>
        </div>
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
          <div className="text-2xl font-serif text-blue-400">{nonEssential.length}</div>
          <div className="text-sm text-neutral-400">Non-essential</div>
          <div className="text-xs text-neutral-500 mt-1">Body can synthesize</div>
        </div>
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
          <div className="text-2xl font-serif text-amber-400">{conditional.length}</div>
          <div className="text-sm text-neutral-400">Conditional</div>
          <div className="text-xs text-neutral-500 mt-1">Essential under stress</div>
        </div>
        <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-4">
          <div className="text-2xl font-serif text-purple-400">{terminator.length}</div>
          <div className="text-sm text-neutral-400">Terminator</div>
          <div className="text-xs text-neutral-500 mt-1">Stop signal for translation</div>
        </div>
      </section>
      ...
      <section>
        <h2 className="font-serif text-xl mb-4 flex items-center gap-2">
          ...
          <span className="text-sm text-neutral-500">({essential.length})</span>
        </h2>
      ...
      <section>
        <h2 className="font-serif text-xl mb-4 flex items-center gap-2">
          ...
          <span className="text-sm text-neutral-500">({nonEssential.length})</span>
        </h2>
        <div className="grid md:grid-cols-2 gap-4">
      ...
      <section>
        <h2 className="font-serif text-xl mb-4 flex items-center gap-2">
          ...
          <span className="text-sm text-neutral-500">({conditional.length})</span>
        </h2>
        <div className="grid md:grid-cols-2 gap-4">
      ...
      <section className="flex gap-4 flex-wrap">
        ...
        >
          ...
          <p className="text-sm text-neutral-400 mt-1">The 21 chemical families</p>
        </Link>
        <Link
          to="/gene-keys"
          className="flex-1 min-w-[200px] p-4 bg-neutral-900/50 rounded-xl border border-neutral-800 hover:border-neutral-700 transition-colors"
...
function AminoAcidCard({ acid }: { acid: AminoAcid }) {
  ...
  return (
    <div className={`bg-neutral-900/50 rounded-xl p-5 border border-neutral-800 hover:border-${color}-500/30 transition-colors`}>
      ...
      <div className="space-y-2 text-sm">
        {ring && (
          <div className="flex items-center gap-2">
            <span className="text-neutral-500">Codon Ring:</span>
            <Link
        ...
        <div className="flex items-center gap-2">
          <span className="text-neutral-500">Gene Keys:</span>
          <span className="text-neutral-300">{geneKeyCount} keys</span>
        </div>
      </div>
