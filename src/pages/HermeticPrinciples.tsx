...
function PrincipleRow({ principle }: { principle: HermeticPrinciple }) {
  return (
    <Link
      ...
      className="flex items-start gap-5 p-5 bg-neutral-900/50 rounded-xl border border-neutral-800 hover:border-amber-500/40 transition-all group"
    >
      {/* Number + Symbol */}
      <div className="flex-shrink-0 text-center w-14">
        <div className="text-3xl text-amber-400 font-serif">{principle.symbol}</div>
        <div className="text-neutral-600 text-xs mt-1">{principle.number}</div>
      </div>

      <div className="flex-1 min-w-0">
        <h3 className="font-serif text-lg text-white group-hover:text-amber-300 transition-colors mb-1">
          {principle.name}
        </h3>
        <p className="text-amber-400/70 text-sm italic mb-2">"{principle.statement}"</p>
        <p className="text-neutral-400 text-sm line-clamp-2">{principle.essence}</p>
      </div>

      {/* Latin name badge */}
      <div className="flex-shrink-0">
        <span className="text-xs text-neutral-600 italic">{principle.latinName}</span>
      </div>
    </Link>
  );
}
...
function PrincipleCard({ principle }: { principle: HermeticPrinciple }) {
  return (
    <Link
      to={`/hermetic/${principle.id}`}
      className="bg-neutral-900/50 rounded-xl p-5 border border-neutral-800 hover:border-amber-500/50 hover:bg-neutral-900 transition-all group"
    >
      ...
      <h3 className="font-serif text-base text-white group-hover:text-amber-300 transition-colors mb-2">
        {principle.name}
      </h3>

      <p className="text-amber-400/60 text-xs italic mb-3 line-clamp-2">"{principle.statement}"</p>

      <p className="text-neutral-500 text-xs line-clamp-3">{principle.essence}</p>
    </Link>
  );
}
